...
{
    public partial class PromotionWorkListDetail : System.Web.UI.Page
    {
        ...
        protected static string strjsonlzd;
        string APIpath = "";
        ...
        protected void LoadPromotion()
        {
            ...
            if (lPromotionInfo.Count > 0)
            {
                ...
                txtDescription.Text = lPromotionInfo[0].PromotionDesc;
        ...
        protected void LoadShowTaskButton()
        {
            ...
            string flagwfcompleted = Request.QueryString["FlagWfComplete"];
            if(flagwfcompleted == "Y")
            {
                btnApprove.Visible = false;
                btnRevise.Visible = false;
                btnReject.Visible = false;
        ...
        protected void gvPromotion_RowCreated(object sender, GridViewRowEventArgs e)
        {
            ...
            else
            {
                ...
                if (hidPromotionTypeCode.Text == StaticField.PromotionTypeCode06)
                {
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            ...
            StartWorkFlow(promotioncode, "1", "1", "approve", "Approve");
        }
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            ...
            StartWorkFlow(promotioncode, "1", "1", "reject", "Reject");
        }
        ...
        protected void btnRevise_Click(object sender, EventArgs e)
        {
            ...
            StartWorkFlow(promotioncode, "1", "1", "revise", "Revise");
        }
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("PromotionWorkList.aspx");
            }
        }
        ...
        public void StartWorkFlow(string promotioncode, string OStatus, string ostate, string eventsubmit, string eventwording)
        {
            EmpInfo empInfo = new EmpInfo();
            int? promotionid = 0;
            ...
            if (lpromoInfo.Count > 0)
            {
                ...
                starttime = lpromoInfo[0].StartDate;
                endtime = lpromoInfo[0].EndDate;
                discountamount = lpromoInfo[0].ProductDiscountAmount.ToString();
                minimumqty = lpromoInfo[0].MinimumQty.ToString();
            }
            ...
            using (var client = new WebClient())
            {
                ...
                olist.dataFields.Event = eventsubmit;
                olist.dataFields.Actor = hidEmpCode.Value;
                olist.dataFields.Remark = txtDescription.Text;
                ...
                {
                    UpdatePromotionDescbyApprover(promotionid);

                    

                    // Part create example to send LAZADA when approve promotion completed
                    string entire_store = "Grand Sport";

                    if (eventwording != "Approve")
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                        "alert('" + eventwording + " สำเร็จ" + "');window.location ='../WorkList/PromotionWorkList.aspx';", true);
                    }
                    else
                    {
                        string jsonlaz = "";
                        ...
                        List<PromotionDetailInfo> lpdetailInfo = new List<PromotionDetailInfo>();
                        lpdetailInfo = Getpromotiondetailinfoforshowjson(promotioncode);

                        if(lpdetailInfo.Count > 0)
                        {
                            foreach(var od in lpdetailInfo)
                            {
                                

                                jsonlaz += @"{" + Environment.NewLine +
                                           @"""apply"" : """ + entire_store + @"""," + Environment.NewLine +
                                           @"""sample_skus"" : ""[{\""productid\"":\""""" + od.ProductCode + @"\"",\""skuid\"":\""" + od.PromotionDetailInfoId.ToString() + @"\""}]" + Environment.NewLine +
                                           @"""criteria_type"" : """ + @"Amount""," + Environment.NewLine +
                                           @"""criteria_value"" : ""[\""" + minimumqty + @"""\""]""," + Environment.NewLine +
                                           @"""order_numbers"" : """ + @"20""," + Environment.NewLine +
                                           @"""name"" : """ + od.ProductName + @"""," + Environment.NewLine +
                                           @"""playform_channel"" : """ + @"1""," + Environment.NewLine +
                                           
                                           @"""gift_skus"" : ""[{\""productid\"":\""""" + "" + @"\"",\""skuid\"":\""" + "" + @"\""}]" + Environment.NewLine +
                                           @"""start_time"" : """ + starttime + @"""," + Environment.NewLine +
                                           @"""discount_type"" : """ + @"money""," + Environment.NewLine +
                                           @"""end_time"" : """ + endtime + @"""," + Environment.NewLine +
                                           @"""discount_value"" : """ + @"[\""" + discountamount + @"\""]""" + Environment.NewLine +
                                           @"},";
                            }
                        }

                        jsonlaz = jsonlaz.Remove(jsonlaz.Length - 1);

                        strjsonlzd = "json message" + Environment.NewLine + jsonlaz + @"" + Environment.NewLine + eventwording + " สำเร็จ";

                        lblstringjson.Text = strjsonlzd;
                        lblstringtextonfinishd.Text = "ระบบได้ " + eventwording + " เรียบร้อย";

                        List<EmpMapBu> lMaxLevelsEmpMapBu = new List<EmpMapBu>();
                        lMaxLevelsEmpMapBu = GetMaxLevelsEmpMapBu();

                        string maxlevels = "";

                        if (lMaxLevelsEmpMapBu.Count > 0)
                        {
                            maxlevels = lMaxLevelsEmpMapBu[0].Levels;
                        }

                        if (hidLevels.Value == maxlevels)
                        {
                            lblstringjson.Visible = true;
                            lblstringtextonfinishd.Visible = false;
                        }
                        else
                        {
                            lblstringjson.Visible = false;
                            lblstringtextonfinishd.Visible = true;
                        }

                            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-Message').modal();", true);

                        
                    }

                    // End Part create example to send LAZADA when approve promotion completed
                }
            }
        ...
        protected void UpdatePromotionDescbyApprover(int? promotionid)
        {
            string respstr = "";

            APIpath = APIUrl + "/api/support/UpdatePromotionfromApproverWF";
            ...
            if (hidPromotionSet.Value.Trim() != "Y") // Promotion Product Insert
            {
                using (var wb = new WebClient())
                {
                    ...
                    data["PromotionId"] = promotionid.ToString();
                    data["PromotionDesc"] = txtDescription.Text;
        ...
        protected List<PromotionDetailInfo> Getpromotiondetailinfoforshowjson(string promotioncode)
        {
            string respstr = "";

            APIpath = APIUrl + "/api/support/GetPromotiondetailInfoNoPagingbyCriteria";
        ...
        protected void btn_submitredirectlink_Click(object sender, EventArgs e)
        {
            Response.Redirect("PromotionWorkList.aspx");
        }
        protected List<EmpMapBu> GetMaxLevelsEmpMapBu()
        {
            string respstr = "";

            APIpath = APIUrl + "/api/support/GetMaxLevelsEmpMapBu";
            ...
            List<EmpMapBu> lmaxlevelsempmapbu = JsonConvert.DeserializeObject<List<EmpMapBu>>(respstr);

            return lmaxlevelsempmapbu;
        }
        #endregion
    }
}