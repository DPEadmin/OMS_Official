...
{
    public partial class FFOrderChangeInventory : System.Web.UI.Page
    {
        ...
        protected Boolean Changestatus_NoAnswerOrderforCancel()
        {
            ...
            for (int i = 0; i < gvOrder_NoAnswerOrder.Rows.Count; i++)
            {
                ...
                if (checkbox.Checked == true)
                {
                    ...
                    result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = Lbordercode.Text.Trim(), orderstatus = StaticField.OrderStatus_06, orderstate = StaticField.OrderState_04 }); 
                }
            }
        ...
        protected Boolean Changestatus_NoAnswerOrderforBackOrder()
        {
            ...
            for (int i = 0; i < gvOrder_NoAnswerOrder.Rows.Count; i++)
            {
                ...
                if (checkbox.Checked == true)
                {
                    ...
                    result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = Lbordercode.Text.Trim(), orderstatus = StaticField.OrderStatus_02, orderstate = StaticField.OrderState_12 }); 
                }
            }
        ...
        protected string GetLink(object objCode,object InventCode)
        {
            string strCode = (objCode != null) ? objCode.ToString() : "";
            string StrInventCode = (InventCode != null) ? InventCode.ToString() : "";
            return "<a href=\"../FullfillmentOrderManagement/FFOrderChangeInventoryDetail.aspx?inventorycode="+ StrInventCode + "&OrderCode=" + strCode + "\">" + strCode + "</a>";
        }
        ...
        protected Boolean ValidateSearch()
        {
            ...
            {
                flag = false;
                lblSearchOrderDate_NoAnswerOrder.Text = MessageConst._MSG_PLEASEINSERT + " วันที่สั่งซื้อเท่านั้น";
            }
            ...
            {
                flag = (flag == false) ? false : true;
                txtSearchName_NoAnswerOrder.Text = "";
            }
            else
            {
                flag = false;
                txtSearchName_NoAnswerOrder.Text = MessageConst._MSG_PLEASEINSERT + " ชื่อ - สกุลต้องไม่มีอักขระพิเศษ";
            }
            ...
            {
                flag = (flag == false) ? false : true;
                txtSearchDeliverDateva_NoAnswerOrder.Text = "";
            }
            else
            {
                flag = false;
                txtSearchDeliverDateva_NoAnswerOrder.Text = MessageConst._MSG_PLEASEINSERT + " วันที่จัดส่งเท่านั้น";
            }
            return flag;
        }
    }
