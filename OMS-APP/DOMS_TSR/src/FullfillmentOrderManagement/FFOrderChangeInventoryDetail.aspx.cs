...
{
    public partial class FFOrderChangeInventoryDetail : System.Web.UI.Page
    {
        ...
        string APIpath = "";
        L_OrderChangeInventory resultinv = new L_OrderChangeInventory();
        L_OrderChangestatus result = new L_OrderChangestatus();
        string nameinventBeforeChange = "";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                BindDdlInventory();
                ddlInventory.SelectedValue = Request.QueryString["inventorycode"];
                ddlInventory_Defualt(Request.QueryString["inventorycode"], null);
        ...
        {
            saveOrder();
            Response.Redirect("FFOrderChangeInventory.aspx");
        }
        ...
        {
            // เช็ค field ordernote
            List<OrderInfo> lorderInfo = new List<OrderInfo>();
            lorderInfo = GetOrderNote(Request.QueryString["OrderCode"].Trim());
            string Strordernote = "";
            string Strordernotefix = "";

            if (lorderInfo.Count > 0)
            {
                Strordernotefix = lorderInfo[0].OrderNote;
            }

            //update orderdetail and ordertansport
            Strordernote += "เปลียนคลังจาก " + nameinventBeforeChange + " เป็น " + ddlInventory.SelectedItem.Text;
            resultinv.L_OrderChangeInventoryInfo.Add(new OrderChangeInventoryInfo()
            {
                updateBy = hidEmpCode.Value.ToString(),
                ordercode = Request.QueryString["OrderCode"].Trim(),
                Inventorycode = ddlInventory.SelectedValue.ToString(),
                orderstate = StaticField.OrderState_01, 
                orderstatus = StaticField.OrderStatus_01, 
                Inventorycode_Old = Request.QueryString["inventorycode"].ToString(),
                OrderNote = Strordernotefix
            ...
            APIpath = APIUrl + "/api/support/OrderChangeInventory";
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
                
                var jsonObj = JsonConvert.SerializeObject(new { resultinv.L_OrderChangeInventoryInfo });
            ...
            if (ValidateInsert())
            {
                ...
                foreach (var od in filteredList.ToList())
                {
                    ...
                    if (liInfo.Count > 0) // Update
                    {
                        ...
                        int? current = qty - reserved;
                        int? balance = od.Balance;
                ...
                if (sum > 0)
                {
                    ...
                    if (lidInfo.Count > 0) 
                    {
                        foreach (var mon in lidInfo) 
                        {
                            InventoryInfo iInfo = new InventoryInfo();
                            List<InventoryInfo> liInfo = new List<InventoryInfo>();

                            liInfo = GetInventoryDetailInfo(Request.QueryString["inventorycode"].ToString(), mon.ProductCode);
                            if (liInfo.Count > 0) // Update
                            {
                                string respstring = "";
                                APIpath = APIUrl + "/api/support/UpdateInventoryDetailfromTakeOrderRetail";
                                // update คลัง A
                                int? qty = liInfo[0].QTY;
                                int? reserved = liInfo[0].Reserved- mon.Amount;
                                int? current = liInfo[0].Current + mon.Amount;
                                ...
                                using (var wb = new WebClient())
                                {
                                    ...
                                    data["InventoryCode"] = Request.QueryString["inventorycode"].ToString();
                                    data["ProductCode"] = mon.ProductCode;
                                    data["QTY"] = qty.ToString();
                                    data["Reserved"] = reserved.ToString();
                                    data["Current"] = current.ToString();
                                    data["Balance"] = balance.ToString();
        ...
        protected List<InventoryInfo> GetListMasterInventoryByCriteria()
        {
            ...
            using (var wb = new WebClient())
            {
                ...
                data["InventoryCode"] = "";
                data["FlagDelete"] = "N";
                data["MerchantCode"] = Session["MerchantCode"].ToString();
                var response = wb.UploadValues(APIpath, "POST", data);
        ...
        protected void ddlInventory_Defualt(object sender, EventArgs e)
        {
            hidInventorySelected.Value = ddlInventory.SelectedValue;
            ...
            if (lProductInfo.Count > 0)
            {
                BindgvProductInventoryFromOrder(Request.QueryString["inventorycode"], lProductInfo);
            }
            nameinventBeforeChange = ddlInventory.SelectedItem.Text;
        }
        ...
        protected List<OrderInfo> GetOrderNote(string ordercode)
        {
            string respstr = "";

            APIpath = APIUrl + "/api/support/ListOrderNopagingByCriteria";
