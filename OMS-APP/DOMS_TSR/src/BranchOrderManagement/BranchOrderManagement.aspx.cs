...
{
    public partial class BranchOrderManagement : System.Web.UI.Page
    {
        L_OrderChangestatus result = new L_OrderChangestatus();
        string CodelistApprove = "";
        protected static string APIUrl;
        string Codelist_NewOrder = "";
        protected static string tempPathupload = ConfigurationManager.AppSettings["TempPathUpload"].ToString();
        ...
        {
            btnSecondary();
            showSection_PreOrder.CssClass = "btn btn-primary";

            searchSection_PreOrder.Visible = show;
            Section_PreOrder.Visible = show;
        }
        ...
        public void sectionLoad_PreOrder()
        {

            showSection_PreOrder.CssClass = "btn btn-primary";
            showPreOrder(true);

            BindddlSearchOrderStatus_PreOrder();
            LoadOrder_PreOrder();
        ...
        public void btnSecondary()
        {
            ...
            if (check_PreOrder == true) { }
            else { showSection_PreOrder.CssClass = " btn-8bar-disable3"; }
        ...
        protected void showSection_PreOrder_Click(object sender, EventArgs e)
        {
            Hide_Section();
            sectionLoad_PreOrder();
        }
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            else if (msg == PreOrder) 
            {
                check_PreOrder = true;
                showSection_PreOrder.CssClass = "btn btn-danger";
            }
        ...
        public void UpdateRequestForReject_NewOrder(string OrderCode)
        {
            if (ddlOrderRejectStatus_NewOrder.SelectedValue == "-99")
            {
            ...
            else
            {
                ...
                using (var wb = new WebClient())
                {
                    ...
                    data["OrderRejectRemark"] = areaOrderRejectStatus_NewOrder.Text;

                    data["OrderStatusCode"] = StaticField.OrderStatus_07; 

                    data["UpdateBy"] = hidEmpCode.Value;

                    data["ORDERREJECTSTATUS"] = ddlOrderRejectStatus_NewOrder.SelectedValue == "-99" ? data["ORDERREJECTSTATUS"] = "" : data["ORDERREJECTSTATUS"] = ddlOrderRejectStatus_NewOrder.SelectedValue;
        ...
        protected void gvOrder_NewOrder_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            EmpInfo empInfo = new EmpInfo();
            ...
            GridViewRow row = gvOrder_NewOrder.Rows[index];

            HiddenField hidOrderId_NewOrder = (HiddenField)row.FindControl("hidOrderId_NewOrder");
            HiddenField hidOrderCode_NewOrder = (HiddenField)row.FindControl("hidOrderCode_NewOrder");
            HiddenField hidSaleOrderTypeName_NewOrder = (HiddenField)row.FindControl("hidSaleOrderTypeName_NewOrder");
            HiddenField hidCustomerName_NewOrder = (HiddenField)row.FindControl("hidCustomerName_NewOrder");
            HiddenField hidCustomerContact_NewOrder = (HiddenField)row.FindControl("hidCustomerContact_NewOrder");
            HiddenField hidCreateDate_NewOrder = (HiddenField)row.FindControl("hidCreateDate_NewOrder");


            if (e.CommandName == "ShowRequestReject_NewOrder")
            {
                areaOrderRejectStatus_NewOrder.Text = "";

                BindddlOrderRejectStatus_NewOrder();

                lblOrderCode_NewOrder.Text = hidOrderCode_NewOrder.Value;
                lblSaleOrderTypeName_NewOrder.Text = hidSaleOrderTypeName_NewOrder.Value;
                lblCustomerName_NewOrder.Text = "คุณ " + hidCustomerName_NewOrder.Value;
                lblCustomerContact_NewOrder.Text = hidCustomerContact_NewOrder.Value;
                lblOrderDate_NewOrder.Text = hidCreateDate_NewOrder.Value;

                int? sumAmout = sumAmoutOrderDetail_NewOrder(hidOrderCode_NewOrder.Value);
                lblsumAmount_NewOrder.Text = sumAmout != null ? "ราคารวม " + "(" + sumAmout + " รายการ" + ")" : "";
                

                int? sumTotalPrice = sumTotalPriceOrderDetail_NewOrder(hidOrderCode_NewOrder.Value);
                lblsumTotalPrice_NewOrder.Text = sumTotalPrice != null ? String.Format("{0:n}", sumTotalPriceOrderDetail_NewOrder(hidOrderCode_NewOrder.Value)) + " บาท" : "";
                

                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modalRequestForReject').modal();", true);
            }

        }

        protected void btnSubmitRejectForRequest_NewOrder_Click(object sender, EventArgs e)
        {
            UpdateRequestForReject_NewOrder(lblOrderCode_NewOrder.Text);
            reLoadAnySection();
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "alert", "alert('" + MessageConst._INSERT_SUCCESS + "');$('#modalRequestForReject').modal('hide');", true);
        }
        ...
        protected void BindddlOrderRejectStatus_NewOrder()
        {
            string respstr = "";
            APIpath = APIUrl + "/api/support/ListLookupNopagingByCriteria";
            ...
            ddlOrderRejectStatus_NewOrder.DataSource = lLookupInfo;
            ddlOrderRejectStatus_NewOrder.DataTextField = "LookupValue";
            ddlOrderRejectStatus_NewOrder.DataValueField = "LookupCode";
            ddlOrderRejectStatus_NewOrder.DataBind();
            ddlOrderRejectStatus_NewOrder.Items.Insert(0, new ListItem("---- กรุณาเลือก ----", "-99"));
        }
        ...
        #region Pre Order
        #region Function (Pre Order)
        protected void LoadOrder_PreOrder()
        {
            List<OrderInfo> lOrderInfo_PreOrder = new List<OrderInfo>();

            int? totalRow = CountOrderMasterList_PreOrder();

            countSection_PreOrder.Text = totalRow.ToString();

            SetPageBar_PreOrder(Convert.ToDouble(totalRow));

            lOrderInfo_PreOrder = GetOrderMasterByCriteria_PreOrder();

            gvOrder_PreOrder.DataSource = lOrderInfo_PreOrder;
            gvOrder_PreOrder.DataBind();

        }

        public int? CountOrderMasterList_PreOrder()
        {
            string respstr = "";
            ...
            using (var wb = new WebClient())
            {
                ...
                data["OrderCode"] = txtSearchOrderCode_PreOrder.Text;

                data["CreateDate"] = txtSearchOrderDateFrom_PreOrder.Text;

                data["CreateDateTo"] = txtSearchOrderDateUntil_PreOrder.Text;

                data["CustomerCode"] = txtSearchCustomerCode_PreOrder.Text;

                data["CustomerFName"] = txtSearchFName_PreOrder.Text;

                data["CustomerLName"] = txtSearchLName_PreOrder.Text;

                data["OrderTypeCode"] = StaticField.SaleOrderType_02; 
        ...
        public List<OrderInfo> GetOrderMasterByCriteria_PreOrder()
        {
            string respstr = "";
            ...
            using (var wb = new WebClient())
            {
                ...
                data["rowOFFSet"] = ((currentPageNumber_PreOrder - 1) * PAGE_SIZE).ToString();
        ...
        public void clearSearch_PreOrder()
        {
            txtSearchOrderCode_PreOrder.Text = "";
            txtSearchCustomerCode_PreOrder.Text = "";
            txtSearchOrderDateFrom_PreOrder.Text = "";
            txtSearchOrderDateUntil_PreOrder.Text = "";
            txtSearchCustomerCode_PreOrder.Text = "";
            txtSearchFName_PreOrder.Text = "";
            txtSearchLName_PreOrder.Text = "";
        }


        #endregion Function (Pre Order)

        #region Events (Pre Order)
        protected void btnSearch_Click_PreOrder(object sender, EventArgs e)
        {
            LoadOrder_PreOrder();
        }

        protected void btnClearSearch_Click_PreOrder(object sender, EventArgs e)
        {
            clearSearch_PreOrder();
        }


        #endregion Events (Pre Order)

        #region Binding (Pre Order)

        protected void BindddlSearchOrderStatus_PreOrder()
        {
            string respstr = "";
            APIpath = APIUrl + "/api/support/ListLookupNopagingByCriteria";
            ...
            ddlSearchOrderStatus_PreOrder.DataSource = lLookupInfo;
            ddlSearchOrderStatus_PreOrder.DataTextField = "LookupValue";
            ddlSearchOrderStatus_PreOrder.DataValueField = "LookupCode";
            ddlSearchOrderStatus_PreOrder.DataBind();
            ddlSearchOrderStatus_PreOrder.Items.Insert(0, new ListItem("---- กรุณาเลือก ----", "-99"));
        }

        #endregion Binding (Pre Order)

        #region Paging (Pre Order)

        protected void SetPageBar_PreOrder(double totalRow)
        {

            lblTotalPages_PreOrder.Text = Math.Ceiling(totalRow / PAGE_SIZE).ToString(); 

            
            ddlPage_PreOrder.Items.Clear();
            for (int i = 1; i < Convert.ToInt32(lblTotalPages_PreOrder.Text) + 1; i++)
            {
                ddlPage_PreOrder.Items.Add(new ListItem(i.ToString()));
            }
            setDDl_PreOrder(ddlPage_PreOrder, currentPageNumber_PreOrder.ToString());
            

            
            if ((currentPageNumber_PreOrder == 1) && (Math.Ceiling(totalRow / PAGE_SIZE)) > 1)
            {
                lnkbtnFirst_PreOrder.Enabled = false;
                lnkbtnPre_PreOrder.Enabled = false;
                lnkbtnNext_PreOrder.Enabled = true;
                lnkbtnLast_PreOrder.Enabled = true;
            }
            else if ((currentPageNumber_PreOrder.ToString() == lblTotalPages_PreOrder.Text) && (currentPageNumber_PreOrder == 1))
            {
                lnkbtnFirst_PreOrder.Enabled = false;
                lnkbtnPre_PreOrder.Enabled = false;
                lnkbtnNext_PreOrder.Enabled = false;
                lnkbtnLast_PreOrder.Enabled = false;
            }
            else if ((currentPageNumber_PreOrder.ToString() == lblTotalPages_PreOrder.Text) && (currentPageNumber_PreOrder > 1))
            {
                lnkbtnFirst_PreOrder.Enabled = true;
                lnkbtnPre_PreOrder.Enabled = true;
                lnkbtnNext_PreOrder.Enabled = false;
                lnkbtnLast_PreOrder.Enabled = false;
            }
            else
        ...
        private void setDDl_PreOrder(DropDownList ddls, String val)
        {
            ListItem li;
            for (int i = 0; i < ddls.Items.Count; i++)
            {
        ...
        protected void GetPageIndex_PreOrder(object sender, CommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Previous":
                    currentPageNumber_PreOrder = Int32.Parse(ddlPage_PreOrder.SelectedValue) - 1;
                    break;
                ...
                case "Last":
                    currentPageNumber_PreOrder = Int32.Parse(lblTotalPages_PreOrder.Text);
                    break;
            }
        ...
        protected void ddlPage_SelectedIndexChanged_PreOrder(object sender, EventArgs e)
        {
            currentPageNumber_PreOrder = Int32.Parse(ddlPage_PreOrder.SelectedValue);
        ...
        #endregion Function (OrderChanged)

        #region Events (OrderChanged)
        ...
        #endregion Events (OrderChanged)

        #region Binding (OrderChanged)



        #endregion Binding (OrderChanged)

        #region Paging (OrderChanged)
        ...
        #endregion Paging (OrderChanged)

        #endregion OrderChanged

        protected void btnMergeOrder_NewOrder_Click(object sender, EventArgs e)
        {
        ...
        protected void btnMergeOrder_PreOrder_Click(object sender, EventArgs e)
        {
            Changestatus_PreOrder();
            LoadOrder_PreOrder();
            LoadOrder_Cooking();
        }
        ...
        protected Boolean Changestatus_NewOrder()
        {
            for (int i = 0; i < gvOrder_NewOrder.Rows.Count; i++)
            {
                ...
                if (checkbox.Checked == true)
                {
                    ...
                    HiddenField hidSALEORDERTYPE = (HiddenField)gvOrder_NewOrder.Rows[i].FindControl("hidSALEORDERTYPE_NewOrder");
                    ...
                    if(hidSALEORDERTYPE.Value.Trim() == StaticField.SaleOrderType_01) 
                    {
                        result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = hidOrder.Value.ToString(), orderstate = StaticField.OrderState_01, orderstatus = StaticField.OrderStatus_02, FlagApproved = "Y" }); 

                    }
                    else if (hidSALEORDERTYPE.Value.Trim() == StaticField.SaleOrderType_02) 
                    {
                        result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = hidOrder.Value.ToString(), orderstate = StaticField.OrderState_10, orderstatus = StaticField.OrderStatus_02, FlagApproved = "Y" });                  
                    }
        ...
        protected Boolean Changestatus_PreOrder()
        {
            for (int i = 0; i < gvOrder_PreOrder.Rows.Count; i++)
            {
                CheckBox checkbox = (CheckBox)gvOrder_PreOrder.Rows[i].FindControl("chk_PreOrder");

                if (checkbox.Checked == true)
                {
                    HiddenField hidCode = (HiddenField)gvOrder_PreOrder.Rows[i].FindControl("hidOrderId_PreOrder");
                    HiddenField hidOrder = (HiddenField)gvOrder_PreOrder.Rows[i].FindControl("hidOrderCode_PreOrder");
                    ...
                    result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = hidOrder.Value.ToString(), orderstatus = StaticField.OrderStatus_02 }); 
                }
            }
            try
            {
        ...
        protected Boolean Changestatus_SendRider()
        {
            ...
            for (int i = 0; i < gvOrder_Cooking.Rows.Count; i++)
            {
                ...
                if (checkbox.Checked == true)
                {
                    ...
                    result.L_OrderChangestatusInfo.Add(new OrderChangestatusInfo() { updateBy = hidEmpCode.Value.ToString(), ordercode = hidOrder.Value.ToString(), orderstatus = StaticField.OrderStatus_03 }); 
                }
            }
        ...
        protected void chkAll_Change_Cooking(object sender, EventArgs e)
        {
            for (int i = 0; i < gvOrder_Cooking.Rows.Count; i++)
            {

                CheckBox chkall = (CheckBox)gvOrder_Cooking.HeaderRow.FindControl("chkAll_Cooking");
                CheckBox chkProduct = (CheckBox)gvOrder_Cooking.Rows[i].FindControl("chk_Cooking");
        ...
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-Rider').modal();", true);
            
        }

        protected void btnPrint_Cooking_Click(object sender, EventArgs e)
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-Rider').modal();", true);
        }

        protected void btnPrint_Cooking_Click1(object sender, EventArgs e)
        {
            
            Process.Start("C:\\Windows\\Orderdetail.pdf");
            
        }
        private void ViewPdf(string PDF_FileName)
        {
        ...
        protected void btnPrint_Cooking_Click3(object sender, EventArgs e)
        {
            
            Response.Redirect("http://www.microsoft.com");
        }
        protected void btnSubmitRider_Click(object sender, EventArgs e)
        {
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "alert", "alert('" + MessageConst._INSERT_SUCCESS + "');$('#modal-Rider').modal('hide');", true);
            Changestatus_SendRider();
            LoadOrder_Cooking();
            LoadOrder_Cooked();

        }
        protected void btnCanceltRider_Click(object sender, EventArgs e)
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-Rider').modal('hide');", true);
        }
    }
}