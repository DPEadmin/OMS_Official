    ...
    public partial class PO : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                btnShowPODetail.Visible = false;

                BindddlProvince();
                BindddlSearchSupplier();
                BindddlSearchInventory();
                BindddlSearchWorkflowStatus();
                CheckEmpRoleWF();
                LoadPO();
            }
        }
        ...
        protected void btnClearSearch_Click(object sender, EventArgs e)
        {
            ...
            ddlSearchWorkflowStatus.SelectedValue = "-99";
        ...
        protected void btnAddSupplier_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("/src/Purchasing/CreatePO.aspx?hidFlagInsert=" + hidFlagInsert.Value);
        ...
        protected void gvPO_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                Response.Redirect("/src/Purchasing/CreatePO.aspx?hidFlagInsert=" + hidFlagInsert.Value + "&POCode=" + hidPOCode.Value);
            }
        }
        protected void gvPO_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                HiddenField hidWFStatusCode = (HiddenField)e.Row.FindControl("hidWFStatusCode");
                LinkButton btnEdit = (LinkButton)e.Row.FindControl("btnEdit");

                if (hidEmpRole.Value == "Mgr")
                {
                ...
                {
                    if (hidWFStatusCode.Value == "100" || hidWFStatusCode.Value == "400")
                    {
        ...
        protected void chkPOAll_Change(object sender, EventArgs e)
        {
            ...
            {
                CheckBox chkall = (CheckBox)gvPO.HeaderRow.FindControl("chPOAll");

                if (chkall.Checked == true)
                {
                    HiddenField hidCode = (HiddenField)gvPO.Rows[i].FindControl("hidPOId");
                    ...
                    CheckBox chkPO = (CheckBox)gvPO.Rows[i].FindControl("chkPO");
        ...
        protected string GetLink(object objCode)
        {
            ...
            return "<a href=\"PODetail.aspx?POCode=" + strCode + "\">" + strCode + "</a>";
        }
        #endregion

        #region function
        protected void LoadPO()
        {
        ...
        protected List<POInfo> GetPOMasterByCriteria()
        {
            ...
            using (var wb = new WebClient())
            {
                ...
                data["StatusCode"] = ddlSearchWorkflowStatus.SelectedValue;
        ...
        public List<WorkflowInfo> GetWorkflowStatusInfo()
        {
            string respstr = "";

            APIpath = APIUrl + "/api/support/ListWorkflowStatusNoPagingByCriteria";
            using (var wb = new WebClient())
            {
                var data = new NameValueCollection();
                data["FlagDelete"] = "N";
            ...
            List<WorkflowInfo> lWorkflowInfo = JsonConvert.DeserializeObject<List<WorkflowInfo>>(respstr);

            return lWorkflowInfo;
        }
        ...
        protected Boolean ValidateInsertandUpdate()
        {
            ...
            {
                flag = false;
                lblFaxNoIns.Text = "Please insert แฟกซ์ของผู้ผลิต";
            }
            else
            {
                flag = (!flag) ? false : true;
        ...
        protected Boolean DeleteSupplier()
        {
            ...
            for (int i = 0; i < gvPO.Rows.Count; i++)
            {
                CheckBox checkbox = (CheckBox)gvPO.Rows[i].FindControl("chkPO");
            ...
            {
                string respstr = "";
                APIpath = APIUrl + "/api/support/DeletePO";

                using (var wb = new WebClient())
                {
                    var data = new NameValueCollection();

                    data["POCode"] = Codelist;
        ...
        protected void CheckEmpRoleWF()
        {
            List<EmpInfo> lempInfo = new List<EmpInfo>();
            lempInfo = GetEmpRole(hidEmpCode.Value);

            if (lempInfo.Count > 0)
            {
                hidEmpRole.Value = lempInfo[0].RoleCode.Trim();
            }
            ...
            if (hidEmpRole.Value == "Mgr")
            {
                ...
                btnShowPODetail.Visible = true;
            }
        }
        protected List<EmpInfo> GetEmpRole(string empcode)
        {
            string respstr = "";
            APIpath = APIUrl + "/api/support/ListEmpRoleByCriteria_01";
        ...
        protected void BindddlSearchWorkflowStatus()
        {
            List<WorkflowInfo> lWorkflowInfo = new List<WorkflowInfo>();
            lWorkflowInfo = GetWorkflowStatusInfo();

            ddlSearchWorkflowStatus.DataSource = lWorkflowInfo;
            ddlSearchWorkflowStatus.DataValueField = "WFStatusCode";
            ddlSearchWorkflowStatus.DataTextField = "WFStatusName";
            ddlSearchWorkflowStatus.DataBind();

            ddlSearchWorkflowStatus.Items.Insert(0, new ListItem("กรุณาเลือก-------------------------------", "-99"));
        }
